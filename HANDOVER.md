...
2. **service worker** — 매니페스트만으로는 오프라인으로 "열리지" 않는다. 노트는 이미 기기에 있고
   셸 캐시만 없는 상태. 홈 화면 아이콘을 눌렀는데 신호가 없으면 빈 화면이 뜬다. (`PARITY.md` P0-8)
3. **`PARITY.md`의 P0** — 마크다운 키보드 편집 → 팔레트 → 표시 필드 3종 → 폴더·즐겨찾기 →
   다이나리스트 이주 → 검색 연산자와 필터 뷰. 이 순서에 의미가 있다(앞이 뒤의 전제).

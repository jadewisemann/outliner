...
- **⌘A는 테이블 밖에 있었고, 그대로 두었다.** 행 선택 상태의 ⌘C·⌘X·Space가 이미
  `useRowSelection`에 직접 박혀 있어서 ⌘A만 `keymap.ts`로 올리면 그 넷이 갈라진다. editing.md의
  "단축키는 테이블 하나"는 재바인딩 가능한 키를 말하는 것이고, 선택 상태의 키들은 원래 그
  바깥이었다. 이 판정을 editing.md에 적어 두었다.
- **텍스트에서 빠져나오는 조건을 "이미 다 잡았을 때"로 정했다.** 첫 누름을 가로채면 텍스트
  전체 선택이라는 표준 동작을 잃는다. `selectionStart === 0 && selectionEnd === value.length`가
  참일 때만 `preventDefault`하고 행 선택으로 넘긴다. 빈 행은 이 조건이 언제나 참이라 첫 누름에서
  바로 빠져나오는데, 잡을 글자가 없으므로 잃는 것이 없다.
- **넓어지지 않는 단계를 건너뛰지 않으면 죽은 누름이 생긴다.** 자식 없는 외동은 "자기 부분
  트리"와 "자기가 속한 목록"이 둘 다 자기 자신이다. `widerScope`가 후보를 만들 때마다 크기가
  실제로 커졌는지 보고, 커지지 않으면 다음 후보로 넘어간다.
- **`widerScope`는 `Doc`이 아니라 `Row[]`를 받는다.** 호출부가 이미 들고 있는 것이 렌더된 행
  배열이고, 거기에는 제자리 필터가 반영되어 있다. `Doc`을 받으면 `visibleRows`를 필터 없이 다시
  계산하게 되어 화면에 없는 행까지 잡힌다.

## 2026-08-22 - 세로 이동이 레벨을 넘어가게 바꾼 판정

- **Dynalist는 오히려 멈추는 쪽이었다.** 사용자가 "다이나리스트와 달리 우리는 멈춘다"고
  지적해서 확인해 보니, 검색으로 닿는 범위에서는 Dynalist의 ⌘⇧↑↓이 부모 목록 밖으로 나가지
  못하는 것이 의도된 설계이고, "부모를 넘어 옮기게 해 달라"는 기능 요청과 Shift+Tab·Ctrl+Down·
  Tab을 묶은 매크로 회피법이 포럼에 남아 있었다. 즉 **바꾸기 전 동작이 Dynalist와 같았다.**
  egress 제한 때문에 `talk.dynalist.io`와 `help.dynalist.io` 원문에는 닿지 못했고 검색 결과
  요약만 본 상태이므로, 이 근거는 1차 자료가 아니다.
- **그래도 바꿨고, 그것은 사용자의 결정이다.** AGENTS.md의 "동작 차이를 조용히 개선하지
  않는다"에 따라 차이를 먼저 알리고, 두 가지 규칙(부모 밖으로만 나가는 워크플로위 방식과,
  그려진 행 순서를 그대로 따르는 방식) 중에서 사용자가 후자를 골랐다. 그러므로 이 항목은
  Dynalist 호환이 아니라 **의도한 이탈**이고, parity.md §2.1의 "이미 동등하거나 낫다"는 서술은
  구현 전 분석이라 그대로 둔다.
- **선택지를 물을 때 그린 예시가 실제 구현보다 한 단계 굼떴다.** 예시에서는 마지막 자식이
  아래로 갈 때 먼저 부모 밖으로 나오고(화면상 자리는 그대로) 다음 누름에서 아래 항목의 첫
  자식으로 들어갔는데, 그러면 한 번의 누름이 행을 하나도 지나가지 못한다. 선택지의 설명문이
  약속한 "한 행씩 훑고 지나간다"를 지키려고 **한 누름에 정확히 한 행**으로 구현했다.
- **`visibleRows`를 부르지 않고 O(깊이)로 계산한다.** 일괄 이동이 선택 행마다 이 계산을
  부르므로, 매번 문서 전체를 훑으면 큰 문서에서 제곱이 된다. draft 하나를 공유하는 기존 bulk
  구조를 그대로 쓰려면 계산도 draft(`Nodes`)를 직접 읽어야 했다.
- **`moveVertically`의 인자에 `zoomId`가 붙었다.** 벽이 형제 목록의 끝에서 확대 범위의 끝으로
  옮겨갔기 때문에, 이제 확대 상태를 모르면 항목을 확대 밖으로 내보낼 수 있다. 그것은 화면에서
  행이 사라지는 것으로 보인다. e2e에 그 경우를 고정해 뒀다.

...
// ⚠️ 이 저장소에 맞게 수정할 유일한 부분.
// isSensitive: 설계 검토가 필요한 코드 경로. docPrefixes: "문서를 만졌다"로 인정할 경로.
const SCOPES = [
  {
    name: 'outliner',
    designDoc: 'DESIGN.md',
    isSensitive: (p) =>
      p.startsWith('src/') || p.startsWith('api/') || p === 'index.html',
    docPrefixes: ['DESIGN.md', 'IMPLEMENTATION_NOTES.md', 'PLANS.md', 'docs/'],
  },
]

...
([parity.md](../parity.md) §7). 대가는 *타입 정직성*이다 — "여기서 어떤 필드가 진짜냐"를 컴파일러가 아니라
`realDocs()` 같은 관습이 답한다.
...
> **2026-08-20 해소:** 아래 셋 중 1번으로 결정 — 선을 긋고 문서·UI에 명시했다.
> 근거와 대안 기각 이유는 [ADR-0005](../adr/0005-backend-capability-line.md).
...
**권장은 1번.** 2번은 DESIGN.md의 "REST는 URL 하나가 계약"을 깬다.

...
이 문서는 에이전트가 한국어를 출력할 때 준수해야 하는 지침의 정본입니다.
[AGENTS.md](../AGENTS.md)의 「절대 규칙」이 이 문서를 가리키며, **작업 종류와 무관하게 항상
적용됩니다.** 「필요한 것만 연다」는 하위 문서 규칙의 예외입니다.

이 문서는 `fluent-korean` 플러그인(`snflkd/fluent-korean`)의 본문을 그대로 옮긴 것입니다.
플러그인은 `keep-coding-instructions: true`로 선언되어 있으므로, Claude Code의 코딩 지침은
그대로 유지됩니다. 본문을 요약하지 않고 전문을 보존하는 이유는 「상황과 목표」 절에 직접
서술되어 있습니다.

이 저장소에서 이 지침을 어떻게 적용하는지는 맨 아래 「이 저장소에서의 적용 범위」에
정리되어 있습니다. 그 절만 이 저장소가 추가한 내용이고, 나머지는 원문입니다.
...
이 절은 원문이 아니고, 위 지침을 이 저장소에 적용하면서 내린 결정을 기록한 것입니다.

1. **적용 대상은 사용자에게 전달되는 한국어 산문입니다.** 대화 응답, 그리고
   `DESIGN.md`·`PLANS.md`·`README.md`·`docs/` 같은 한국어 문서의 본문이 여기에 해당합니다.

2. **코드에 속하는 텍스트에는 적용하지 않습니다.** 「동작 범위」 2번 조항이 지시하는 대로
   기존 관례를 따릅니다. 이 저장소의 기존 관례는 다음과 같습니다.

   - 코드 주석과 JSDoc은 영어로 작성합니다.
   - 커밋 메시지는 `<type>: <제목>` 형식이며 본문은 한국어로 작성합니다.
   - UI 문자열과 테스트 이름은 각 파일의 기존 언어를 따릅니다.

3. **엠대시 관련 조항은 새로 쓰거나 실질적으로 고치는 산문에만 적용합니다.** 기존 한국어
   문서에는 엠대시가 200군데 이상 사용되어 있는데, 이것을 일괄 치환하면 의미 변화가 없는
   거대한 diff가 발생합니다. AGENTS.md의 「강제되는 것은 문서 수정이 아니라 문서와의 일관성
   검토다」라는 원칙과 「의미 없는 문서 diff를 만들지 않는다」는 지시에 따라, 소급 적용은
   하지 않습니다. 어떤 문서를 다른 이유로 고칠 때 그 문단의 엠대시를 함께 정리하는 것은
   권장됩니다.

4. **사용자 수준 전역 지침에도 같은 내용을 두려면**, 이 파일의 내용을 각 기기의
   `~/.claude/CLAUDE.md`에 넣으면 됩니다. 그렇게 하면 이 저장소 밖의 작업에도 적용됩니다.
   원격 세션 컨테이너에 기록한 전역 설정은 컨테이너가 회수될 때 함께 사라지기 때문에,
   저장소에 보관된 이 파일이 실질적인 정본입니다.

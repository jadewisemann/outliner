...
- indentation이 있는 여러 줄 텍스트를 depth가 있는 노드 draft로 파싱한다.
- 여러 줄 paste가 현재 노드를 split하고, 붙여넣은 outline 구조를 대상 노드 뒤에 삽입한다.
- `Shift+ArrowUp`/`Shift+ArrowDown`이 visible node 기준 연속 범위를 선택한다.
- 접힌 subtree 내부 노드는 다중 선택 범위에 포함되지 않는다.
- 부모와 자식이 함께 선택되면 부모만 최상위 작업 대상으로 정규화한다.
- 선택된 여러 최상위 노드가 순서를 유지한 채 bulk indent된다.
- 선택된 여러 최상위 노드가 순서를 유지한 채 bulk outdent된다.
- 선택된 여러 최상위 subtree가 bulk delete된다.
- 선택된 여러 노드를 indentation 기반 plain text로 복사한다.

### 구현 항목

- `src/domain/bulkOutline.ts`
- `src/domain/bulkOutline.test.ts`
- `parseIndentedText`
- `insertNodesFromText`
...
- UI selection state 확장: `anchorNodeId`, `focusNodeId`, `selectedNodeIds`
